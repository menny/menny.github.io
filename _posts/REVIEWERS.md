...
## 4. Fact-Checking
*   **Brave Search MCP:** Use the `brave-search` tool to verify quantitative claims, dates, library version support, and benchmark results.
*   **Link Verification:** For all citation links, use the `brave-search` to confirm that the link points to the intended content and that the cited information accurately reflects the linked source.
