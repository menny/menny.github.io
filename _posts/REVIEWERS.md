...
## 4. Fact-Checking
*   **Brave Search MCP:** Use the `brave-search` tool to verify quantitative claims, dates, library version support, and benchmark results. Do not use it for direct URL fetching.
*   **Link Verification:** For all citation links, use the `fetch` tool to confirm that the link points to the intended content and that the cited information accurately reflects the linked source.
